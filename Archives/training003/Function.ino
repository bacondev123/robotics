...
void trackCross_PID_Offset(float kp, float ki, float kd, unsigned long leave_time) {
  ...
  // Back down to 250! The "center" trick will now safely survive B1.
  unsigned long start_time = millis(); 

  // ============================================
  // PHASE 1: Find the REAL cross intersection
  // ============================================
  while (1) {
    readSensors();

    // 1. Is it mathematically a cross line physically?
    bool cross = (sensorValue[0] < sensorRef[0]) && (sensorValue[3] < sensorRef[3]);

    // 2. NEW MAGIC: Is the robot's heading steady? 
    // Corners throw massive 2.0+ error ratings. Straights throw 0 to 1.0!
    bool is_centered = (abs(error) <= 1.2);

    // Only if BOTH are true can we safely stop line-following!
    if (cross && is_centered && (millis() - start_time > 250)) {
      break; 
    }
  ...
  while (millis() - cross_time < leave_time) {
    fd2(basespeed, basespeed + 2); // Maintain steady momentum
  }
