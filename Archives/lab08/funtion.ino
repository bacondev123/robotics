...
void fd_c(int sensor, int sensor_ref) {             // เดินหน้าจนกว่าจะเจอเส้น แล้ว หยุด
  while (1) {                       // ให้ทำคำสั่ง วนไปตลอดไม่หยุด
    if (analog(sensor) < sensor_ref) {  //  ถ้า analog(0) < 600 คือ เจอดำ
      break;                        // ให้ ออกจาก loop
    }
...
void Track_Side_Line_PID(int sensor, float basespeed, float KpTemp, float KiTemp, float KdTemp){

  int ref = get_sensor_ref(sensor);

  while(true){
    if(analog(sensor) < ref){
      break;
    }else{
      trackline_PID(basespeed, KpTemp, KiTemp, KdTemp);
    }
  }
  PID_Reset();
  ao();
