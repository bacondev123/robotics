...
// =================================================================
// PAIRED MOTOR & BUZZER TESTER
// =================================================================

void testOnePair(int pinA, int pinB, String pairName) {
  displayPrint("Testing:\n" + pairName, 0, 10, true);
  Serial.println("\nTesting: " + pairName);

  pinMode(pinA, OUTPUT);
  pinMode(pinB, OUTPUT);

  // 1. Test Forward (Pin A high, Pin B grounded)
  digitalWrite(pinB, LOW);
  analogWrite(pinA, 150); // 60% power
  delay(800);
  analogWrite(pinA, 0);
  delay(300);

  // 2. Test Backward (Pin B high, Pin A grounded)
...
void testLeftMotorAndBuzzer() {
  displayPrint("TEST START\nPress SW1", 0, 10, true);
  while (digitalRead(8) == HIGH) { delay(10); } // Wait for SW1
  while (digitalRead(8) == LOW)  { delay(10); }

  // Test candidate pairs for Left Motor (M1)
  testOnePair(11, 12, "Pair 11 & 12");
  testOnePair(10, 11, "Pair 10 & 11");
  testOnePair(15, 16, "Pair 15 & 16");
  testOnePair(2, 3,   "Pair 2 & 3");
  testOnePair(6, 7,   "Pair 6 & 7");

  // Test candidate pins for Buzzer
  const int buzzerCandidates[] = { 10, 15, 18, 19, 21, 22 };
  for (int i = 0; i < 6; i++) {
    int bPin = buzzerCandidates[i];
    displayPrint("Testing Buzzer\nPin: " + String(bPin), 0, 10, true);
    tone(bPin, 2000, 200);
    delay(500);
  }

  displayPrint("TEST DONE!", 0, 20, true);
}
