...
void followUntilSensorDetected(int sensorIndex) {
  ...
  while (1) {
    ...
    if (isBlack(sensorIndex)) {
      ...
      hitCount = 0; 
      driveLineFollowerPID(maxSpeed, kP, kI, kD);
    }
  }
  ao();
  delay(LOOP_DELAY_MS);
...
void driveForwardConstant(unsigned long travelTimeMs) {
  ...
  int targetSpeed = compensateSpeed(20);
  unsigned long startTime = millis();
  while (millis() - startTime < travelTimeMs){
    fd2(targetSpeed + MOTOR_BIAS, targetSpeed);
  }
  ao();
  resetPIDVariables();
  delay(LOOP_DELAY_MS);
